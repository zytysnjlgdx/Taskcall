...
def get_ready_node_ids(
    ...
    for node in plan.nodes:  # 一个节点一个节点地检查
    ...
    return ready_node_ids  # 返回当前可以执行的节点id列表
...
def has_unfinished_nodes(node_states: dict[str, NodeRuntimeState]) -> bool:
    ...
    return any(state.status in {"pending", "ready", "running"} for state in node_states.values())  # 只要里面有一个条件为 True，整体就返回 True；全部都是 False 才返回 False
